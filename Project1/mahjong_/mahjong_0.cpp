...
int paichi[50]; //未出现的牌剩余的数量,用以估价,花牌不考虑
...
inline void Deletehand(string &s_)
{
...
void PENG(int card_)
{
    int count = 0;
    string tmpCard = Getstr(card_);
    for (auto iter = hand.begin() + frozen; iter != hand.end();)
    {
        if (Getint(*iter) == card_)
        {
...
{
    //首先实现将刚拿到的牌成组冻结
    sort(hand.begin() + frozen, hand.end());
    ...
    //估价准备弃牌
    int int_hand[50];
    memset(int_hand, 0, sizeof(int_hand));
    for (auto iter = hand.begin() + frozen; iter < hand.end(); ++iter)
    {
        ++int_hand[Getint(*iter)];
    }
    return *(hand.rbegin());
}
