...
//特定番形分析所需参数
//清龙一色三步高
bool None_SS_Mianzi = 0; //三面子番形除了番形之外的一个面子
...
bool CanPENG(int who_, tile_t card_) //判定能不能碰
{
    ...
    case QINGLONG:
        ...
        else if (tile_get_suit(card_) == mainColor)
        {
            ret = 0;
        }
        if (ret)
    ...
    case YISESANBUGAO:
        ...
        {
            if (tile_get_rank(card_) > 1 && tile_get_rank(card_) < 9)
            {
                ret = 0;
            }
        }
    ...
    case SANSESANBUGAO:
        ...
        else
        {
            ...
            {
                if (tile_get_rank(card_) < SS_CentralRank + 2 && tile_get_rank(card_) > SS_CentralRank - 2)
                {
                    ret = 0;
                }
            }
...
bool CanCHI(int who_, tile_t card_, tile_t &retMidCard)
{
    ...
    case SANSESANTONGSHUN:
        ...
        if (tile_get_rank(retMidCard) == SS_CentralRank ||
            tile_get_rank(retMidCard) == SS_CentralRank - 1 ||
            tile_get_rank(retMidCard) == SS_CentralRank + 1)
        {
            if (handCount[card_])
            {
            ...
            else
            {
                ...
                {
                    if (tile_get_suit(pack_get_tile(tmp_pack)) == tile_get_suit(retMidCard))
                    {
                        ret = 0;
                    }
                }
...
void SortHand_QLYS()
{
    ...
    else if (selfTargetFan == YISESANBUGAO)
    {
        for (tile_t a : all_tiles)
        {
            ...
            {
                QLYSvalue[a] += 0.65 * handCount[a - 2] + 0.15 * handCount[a + 2];
            }
            if (tile_get_rank(a) < 9 && (handCount[a - 1] + unknown[a - 1]) > 0)
            {
                QLYSvalue[a] = handCount[a + 2] * 0.65 + unknown[a + 2] * 0.15;
            }
        }
    }
...
void SortHand_QIDUI()
{
    ...
    for (tile_t a : all_tiles)
    {
        ...
        {
            if (tile_get_rank(a) == 1 || tile_get_rank(a) == 9)
            {
                QDvalue[a] *= 1.05;
            }
            else if (tile_get_rank(a) == 2 || tile_get_rank(a) == 8)
            {
                QDvalue[a] *= 1.03;
            }
        }
    ...
    for (tile_t a : hand)
    {
        if (WMvalue[a] >= 0 && WMvalue[a] < 100 && tile_get_suit(a) <= 3)
        {
            ...
            else
            {
                ...
                if (tile_get_rank(a) > 1 && (handCount[a - 1] + unknown[a - 1]) > 0)
                {
                    WMvalue[a] += 0.65 * handCount[a - 2] + 0.15 * handCount[a + 2];
                }
                if (tile_get_rank(a) < 9 && (handCount[a - 1] + unknown[a - 1]) > 0)
                {
                    WMvalue[a] = handCount[a + 2] * 0.65 + unknown[a + 2] * 0.15;
                }
            }
        }
...
void SortHand_SS()
{
    ...
    for (tile_t a : hand)
    {
        if (tile_get_suit(a) > 3)
        {
            ...
            {
                SSvalue[a] = (handCount[a] >= 2) * 20 * (unknown[a] >= 1);
            }
        }
        else
        {

            if (tile_get_rank(a) >= SS_CentralRank - 1 && tile_get_rank(a) <= SS_CentralRank + 1)
            {
                SSvalue[a] = 100;
                if (handCount[a] > 1)
                {
                    SSvalue[a] = handCount[a] * 0.8 + unknown[a] * 0.3 +
                                 (handCount[a - 1] + handCount[a + 1]) * 0.9 + (unknown[a - 1] + unknown[a + 1]) * 0.4;
                    if (tile_get_rank(a) > 1 && (handCount[a - 1] + unknown[a - 1]) > 0)
                    {
                        SSvalue[a] += 0.65 * handCount[a - 2] + 0.15 * handCount[a + 2];
                    }
                    if (tile_get_rank(a) < 9 && (handCount[a - 1] + unknown[a - 1]) > 0)
                    {
                        SSvalue[a] = handCount[a + 2] * 0.65 + unknown[a + 2] * 0.15;
                    }
                }
                for (pack_t p : mingPai[playerID])
                {
                    if (tile_get_suit(pack_get_tile(p)) == tile_get_suit(a) && tile_get_rank(pack_get_tile(p)) == SS_CentralRank)
                    {
                        SSvalue[a] = handCount[a] * 0.8 + unknown[a] * 0.3 +
                                     (handCount[a - 1] + handCount[a + 1]) * 0.9 + (unknown[a - 1] + unknown[a + 1]) * 0.4;
                        if (tile_get_rank(a) > 1 && (handCount[a - 1] + unknown[a - 1]) > 0)
...
#ifdef _DEBUG_
        cerr << "SSvalue " << GetStr(a) << SSvalue[a] << '\n';
#endif
    }
