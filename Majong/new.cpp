...
void SortHand_QLYS()
{
    ...
    else if (selfTargetFan == YISESANBUGAO)
    {
        for (tile_t a : all_tiles)
        {
            ...
            QLYSvalue[a] = handCount[a] * 0.75 + unknown[a] * 0.25 +
                           (handCount[a - 1] + handCount[a + 1]) * 0.8 + (unknown[a - 1] + unknown[a + 1]) * 0.3;

            if (tile_get_rank(a) > 1 && (handCount[a - 1] + unknown[a - 1]) > 0)
            {
                QLYSvalue[a] += 0.7 * handCount[a - 2] + 0.2 * handCount[a + 2];
            }
            if (tile_get_rank(a) < 9 && (handCount[a + 1] + unknown[a + 1]) > 0)
            {
                QLYSvalue[a] = handCount[a + 2] * 0.7 + unknown[a + 2] * 0.2;
            }
        }
    }
