...
//本局输入，进行决策,如果长时运行，前两局可以不读
    ...
    {
#ifdef _LONG_TIME_
        int tmp_len=inputStr.length();
        getline(cin, inputStr);

#ifdef _DEBUG_
        cerr <<"GetInputStr:"<< inputStr<<endl;
#endif    
        if (turnID > 0)
        {
            inputStr = "{\"requests\":[" + inputStr + "\"]}";
#ifdef _DEBUG_
            cerr <<"GetJSONStr:"<< inputStr<<endl;
        ...
        readfrom=0;
#endif
        requestsVector.push_back(inputJSON["requests"][readfrom].asString());
        istringstream in(inputJSON["requests"][readfrom].asString());
        ostringstream out;
        in >> type;
        switch (type)
        {
