...
void changeTarget()//判定是否开始碰碰胡
{
    if(tileWall<40&&selfTargetFan==QIDUI)
    {
        if(seven_pairs_shanten(&hand[0],hand.size(),nullptr)>=3)
        {
            selfTargetFan=PENGPENGHU;
        }
    }
}
...
void SortHandQD()
{
    ...
    for(tile_t a:all_tiles)
    {
        ...
        {
            QDvalue[a]=(unknown[a])*(unknown[a]-1)/(double)((unknownSum)*(unknownSum-1))*0.9;//延长和牌时间有风险
        }
    }
...
{
    //超过两个则优先打出，再按剩余的牌数估计机会
    static unordered_map<tile_t,double> PPHvalue;//估价数组
    int unknownSum=0;
    for(tile_t a:all_tiles)
    {
        unknownSum+=unknown[a];
        PPHvalue[a]=unknown[a];
        handCount[a]=0;
    }
    for(tile_t a:hand)
    {
    ...
    for(tile_t a:all_tiles)
    {
        ...
        else if(handCount[a]==2)
        {
            ...
            {
                PPHvalue[a]=100*(1+2*unknown[a])*(1+abs(tile_get_rank(a)-5)*0.1);
            }
            else
            {
                PPHvalue[a]=100*(1+2*unknown[a])*1.5;
            }
    ...
    sort(hand.begin(),hand.end(),
    [&PPHvalue](tile_t a,tile_t b)
    {
        return PPHvalue[a]>PPHvalue[b];
    }
    );
}
...
tile_t Discard(vector<tile_t> forbid_ = vector<tile_t>()) //估价弃牌
{
    ...
    else if(selfTargetFan==PENGPENGHU)
    {
        SortHandPPH();
    }
    if (forbid_.empty())
