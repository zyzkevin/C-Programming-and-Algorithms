...
void SortHand_QLYS()
{
    ...
    else if (selfTargetFan == YISESANBUGAO)
    {
        for (tile_t a : all_tiles)
        {
            ...
            QLYSvalue[a] *= 1 - 0.05 * (abs(tile_get_rank(a) - 5));
            QLYSvalue[a] *= 1 + 0.05 * (3 - unknown[a]);
        }
    }
...
void SortHand_WUMENQI()
{
    ...
    for (pack_t p : mingPai[playerID])
    {
        ...
        else
        {
            if (tile_get_rank(pack_get_tile(p)) <= 4)
            {
                ...
                {
                    WMvalue[make_tile(tmp_suit, tmp_rank)] = 0;
                }
            }
            else
            {
    ...
    for (tile_t a : hand)
    {
        if (WMvalue[a] && tile_get_suit(a) <= 3)
        {
            if (handCount[a] * handCount[a + 1] * handCount[a + 2])
            {
                for (rank_t i = 1; i <= 9; ++i)
                {
                    WMvalue[make_tile(tile_get_suit(a), i)] = 0;
                }
                WMvalue[a] = 100;
                WMvalue[a + 1] = 100;
                WMvalue[a + 2] = 100;
            ...
            else
            {
                WMvalue[a] = handCount[a] * 0.8 + unknown[a] * 0.3 +
                             ...
                             (handCount[a - 2] + handCount[a + 2]) * 0.85 + (unknown[a - 2] + unknown[a + 2]) * 0.35;
            }
        }

        if (WMvalue[a] && tile_get_suit(a) == TILE_SUIT_HONORS)
        {
            if (handCount[a] > 3)
            {
                if (tile_get_rank(a) <= 4)
                {
                    ...
                    {
                        WMvalue[make_tile(TILE_SUIT_HONORS, i)] = 0;
                    }
                    WMvalue[a] = 0.5;
                }
                else
            ...
            {
                if (WUMEN_Count[4] + WUMEN_Count[5])
                {
                    for (rank_t i = 1; i <= 9; ++i)
                    {
                        WMvalue[make_tile(tile_get_suit(a), i)] = 0;
